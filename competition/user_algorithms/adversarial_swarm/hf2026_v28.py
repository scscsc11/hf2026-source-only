"""
HF2026 V28 — staged rendezvous ring + V26 independent consensus
...
No search, SAM routing, helper selection, final 430 m dwell geometry, peer
avoidance, YOLO thresholds, real/decoy classification, or report_target
behaviour is changed in this version.  V26 introduced independent same-vehicle
evidence.  V27 keeps that evidence/consensus layer intact and changes only the
first-lock rendezvous geometry: 300 m / 120 m before consensus, then the proven
430 m / 165 m geometry after consensus / DWELL.
...
Participant path:
competition/user_algorithms/adversarial_swarm/hf2026_v28.py
"""
...
# V26 independent same-vehicle evidence.  Acquisition may use the existing
# broad task prior, but once a local evidence track exists it is updated only
# from this UAV's own Detection stream; owner T packets never correct it.
EVIDENCE_ASSOC_GATE_M = 165.0
EVIDENCE_RESET_GAP_S = 2.4
EVIDENCE_REPORT_SANITY_M = 360.0
CONSENSUS_EVIDENCE_FRESH_S = 1.45
CONSENSUS_PAIR_GATE_M = 150.0
...
# Tracking geometry: keep V15/V21's officially proven 430 m final dwell ring,
# but V27 stages the FIRST rendezvous on a closer 300 m ring.  The three
# 120-degree acquisition anchors are ~520 m apart; with 120 m loiter circles,
# their nominal closest separation remains ~280 m, above the 200 m proximity
# penalty boundary.  Once the triad has ever formed consensus / enters DWELL,
# all members return to the proven 430 m / 165 m geometry.
RENDEZVOUS_ANCHOR_RADIUS_M = 300.0
RENDEZVOUS_LOITER_RADIUS_M = 120.0
...
class Task:
    ...
    # V26: independent local evidence track.  This is deliberately separate
    # from ``filt`` because ``filt`` is the shared navigation/task predictor
    # and is corrected by owner T packets / weak multi-view fusion.
    evidence_filt: Optional[TrackFilter] = None
    evidence_points: Dict[int, Tuple[float, float, float]] = field(default_factory=dict)
...
class HF2026V28Agent(SwarmAgent):
    """V28: V27 geometry/consensus + local-evidence gimbal stickiness."""
    ...
    def configure(self, config) -> None:
        ...
        self._last_gimbal_source_log_t = -999.0
    ...
    def decide(self, obs: SwarmObs, dt: float) -> List[Command]:
        ...
        if self._task is not None:
            assoc = self._task.evidence_filt or self._task.filt
            det = self._best_detection(obs, (assoc.lat, assoc.lon))
    ...
    def _assign_tracking_roles(
        ...
        anchors = []
        # V27 assigns roles against the same 300 m acquisition ring that the
        # three aircraft actually fly during first-lock rendezvous.  This is
        # the only pre-lock geometry change; role bearings are unchanged.
        for base_b in TRACK_ANCHOR_BEARINGS_DEG:
            ...
            anchors.append(
                _offset(
                    target_lat,
                    target_lon,
                    RENDEZVOUS_ANCHOR_RADIUS_M * math.sin(b),
                    RENDEZVOUS_ANCHOR_RADIUS_M * math.cos(b),
                )
            )
    ...
    def _try_start_task(self, obs: SwarmObs) -> None:
        ...
        if helpers is None:
            if self._t - self._last_nohelper_log_t >= 2.0:
                ...
                print(
                    f"[V28][{self.my_uid}] candidate-ready but helpers<2 "
        ...
        evidence_filt = TrackFilter(
            lat=c.filt.lat, lon=c.filt.lon, last_t=self._t,
            last_seen=c.filt.last_seen, v_east=c.filt.v_east,
            v_north=c.filt.v_north, hits=c.filt.hits,
            history=list(c.filt.history),
        )
        ...
        task = Task(
            ...
            evidence_filt=evidence_filt,
            evidence_points={self._idx: (c.filt.last_seen, c.filt.lat, c.filt.lon)},
        )
        ...
        print(
            f"[V28][{self.my_uid}] TASK_START t={self._t:.1f}s "
    ...
    def _accept_task(
        ...
        print(
            f"[V28][{self.my_uid}] TASK_ACCEPT t={self._t:.1f}s "
    ...
    def _update_task_local(self, det: Optional[Tuple[float, float]]) -> None:
        ...
        # Shared task predictor is for navigation / coarse LOS only.
        task_pred = (t.filt.lat, t.filt.lon)
        if t.evidence_filt is not None:
            t.evidence_filt.predict_to(self._t)

        task_compatible = (
            det is not None
            and _dist_m(det[0], det[1], task_pred[0], task_pred[1]) <= TASK_GATE_M
        )
        evidence_compatible = False
        if det is not None and t.evidence_filt is not None:
            evidence_compatible = (
                _dist_m(det[0], det[1], t.evidence_filt.lat, t.evidence_filt.lon)
                <= EVIDENCE_ASSOC_GATE_M
            )

        # Bootstrap from the broad owner task prior, but thereafter the
        # evidence track is independent.  After a >grace-sized loss we allow
        # a clean re-bootstrap; confirmation counters are reset so a vehicle
        # switch cannot inherit old evidence.
        if det is not None and (
            t.evidence_filt is None and task_compatible
            or (t.evidence_filt is not None
                and self._t - t.evidence_filt.last_seen > EVIDENCE_RESET_GAP_S
                and task_compatible and not evidence_compatible)
        ):
            t.evidence_filt = TrackFilter(
                lat=det[0], lon=det[1], last_t=self._t, last_seen=self._t,
                hits=1, history=[(self._t, det[0], det[1])],
            )
            t.local_match_hits = 0
            t.local_match_first = -999.0
            t.local_match_last = -999.0
            t.local_confirmed = False
            t.evidence_points.pop(self._idx, None)
            evidence_compatible = True

        visual_bridge = (
            not evidence_compatible
            and t.evidence_filt is not None
            and t.evidence_filt.last_seen > -900.0
            and self._t - t.evidence_filt.last_seen <= TASK_VISUAL_BRIDGE_GEO_AGE_S
            and self._visual_fresh()
            and self._vision.center_error <= TASK_VISUAL_BRIDGE_CENTER_ERR
            and self._vision.hits >= TASK_VISUAL_BRIDGE_MIN_HITS
        )
        new_visual_support = visual_bridge and self._vision.result_seq != t.local_visual_seq
        support_hit = evidence_compatible or new_visual_support

        if support_hit:
            if t.local_match_hits <= 0 or self._t - t.local_match_last > FOLLOWER_CONFIRM_GAP_S:
                t.local_match_hits = 0
                t.local_match_first = self._t
            t.local_match_hits += 1
            t.local_match_last = self._t

            if evidence_compatible and det is not None and t.evidence_filt is not None:
                t.evidence_filt.correct(self._t, det[0], det[1], alpha=0.34)
                # Local detections may still refine the navigation predictor.
                t.filt.correct(self._t, det[0], det[1], alpha=0.24)
            ...
            locally_confirmed = (
                self._idx == t.owner
                or (t.local_match_hits >= FOLLOWER_CONFIRM_HITS
                    and self._t - t.local_match_first >= FOLLOWER_CONFIRM_SPAN_S)
            )
            if locally_confirmed and t.evidence_filt is not None:
                first_confirm = not t.local_confirmed
                t.local_confirmed = True
                t.visuals[self._idx] = self._t
                t.evidence_points[self._idx] = (
                    self._t, t.evidence_filt.lat, t.evidence_filt.lon
                )
                if first_confirm:
                    print(
                        f"[V28][{self.my_uid}] LOCAL_CONFIRM t={self._t:.1f}s "
                        f"owner={t.owner} seq={t.seq} hits={t.local_match_hits} "
                        f"independent=Y vision={'Y' if self._visual_fresh() else 'N'}"
                    )
                if self._idx != t.owner and self._t - self._last_visual_tx >= 0.68:
                    lat_i, lon_i = _geo_to_int(t.evidence_filt.lat, t.evidence_filt.lon)
                    self._queue(f"V,{t.owner},{t.seq},{self._idx},{lat_i},{lon_i}", priority=1)
    ...
    def _log_follower_status(
        ...
        ring_radius, _ = self._task_geometry(t)
        ...
        _, gsrc, _, ev_sep = self._task_gimbal_target(t)
        ev_sep_s = "NA" if ev_sep is None else f"{ev_sep:.0f}m"

        print(
            f"[V28][{self.my_uid}] FOLLOWER_STATUS t={self._t:.1f}s "
            ...
            f"geo_age={geo_age:.1f}s ev={'Y' if t.evidence_filt is not None else 'N'} "
            f"vision={'Y' if visual_fresh else 'N'} "
            f"vhits={self._vision.hits} verr={visual_err} "
            f"gsrc={gsrc} evsep={ev_sep_s} "
            f"ring={ring_radius:.0f}m target_d={target_d:.0f}m "
            f"anchor_d={anchor_d:.0f}m"
        )
    ...
    def _task_gimbal_target(
        self,
        t: Task,
    ) -> Tuple[Tuple[float, float], str, float, Optional[float]]:
        """Choose this UAV's physical gimbal LOS for an active task.

        V28 keeps ``t.filt`` as the shared navigation / coalition predictor.
        Once this UAV has independently confirmed ``evidence_filt``, however,
        its gimbal follows that local track for up to EVIDENCE_RESET_GAP_S.
        """
        task_target = (t.filt.lat, t.filt.lon)
        ev = t.evidence_filt
        if ev is None:
            return task_target, "TASK", 999.0, None

        ev_age = self._t - ev.last_seen if ev.last_seen > -900.0 else 999.0
        ev_sep = _dist_m(ev.lat, ev.lon, t.filt.lat, t.filt.lon)

        if t.local_confirmed and ev_age <= EVIDENCE_RESET_GAP_S:
            return (ev.lat, ev.lon), "EV", ev_age, ev_sep

        return task_target, "TASK", ev_age, ev_sep
    ...
    def _track_commands(self, obs: SwarmObs) -> List[Command]:
        ...
        role = list(t.members).index(self._idx)
        _, loiter_radius = self._task_geometry(t)
        desired = self._tracking_anchor(obs, t, role)
        desired = self._deconflict_goal(obs, desired)

        # Navigation intentionally remains on the shared task predictor and the
        # unchanged V27 300m->430m staged geometry.
        cmds = self._follow_safe_route(
            obs,
            desired,
            mode=f"track{t.owner}:{t.seq}:{role}",
            speed=TRACK_SPEED_MPS,
            final_loiter_radius=loiter_radius,
            refresh_s=3.0,
            moving_goal_replan_m=95.0,
            max_plan_age_s=8.0,
        )

        # V28 changes ONLY the physical geographic LOS source. Once this UAV
        # independently confirms its local evidence track, aim at that track
        # rather than repeatedly re-centering on the shared task prior.
        gimbal_target, gsrc, ev_age, ev_sep = self._task_gimbal_target(t)
        ...
        if self._t - self._last_gimbal_source_log_t >= 2.0:
            ev_sep_s = "NA" if ev_sep is None else f"{ev_sep:.0f}m"
            ev_age_s = "NA" if ev_age >= 900.0 else f"{ev_age:.1f}s"
            print(
                f"[V28][{self.my_uid}] GIMBAL_SOURCE t={self._t:.1f}s "
                f"owner={t.owner} seq={t.seq} phase={t.phase} role={role} "
                f"src={gsrc} ev_age={ev_age_s} ev_task_sep={ev_sep_s} "
                f"fov={desired_fov:.0f}"
            )
            self._last_gimbal_source_log_t = self._t
    ...
    def _task_geometry(self, t: Task) -> Tuple[float, float]:
        """Return (anchor_radius, loiter_radius) for the current task stage.

        V27 changes only the *pre-lock* rendezvous geometry.  The first-lock
        phase uses a closer ring so a recruited follower can get the commanded
        target into its local detector/gimbal footprint sooner.  After this
        task has ever demonstrated triad consensus, or after D is learned from
        the owner, the historical 430 m final geometry is restored.
        """
        first_lock = t.phase == "R" and not t.ever_all_visual
        if first_lock:
            return RENDEZVOUS_ANCHOR_RADIUS_M, RENDEZVOUS_LOITER_RADIUS_M
        return TRACK_ANCHOR_RADIUS_M, TRACK_LOITER_RADIUS_M
    ...
    def _tracking_anchor(
        ...
        radius, _ = self._task_geometry(t)
    ...
    def _triad_consensus(self, t: Task):
        pts = {}
        ages = {}
        for m in t.members:
            item = t.evidence_points.get(m)
            if item is None:
                ages[m] = 999.0
                continue
            ts, la, lo = item
            ages[m] = self._t - ts
            if ages[m] <= CONSENSUS_EVIDENCE_FRESH_S:
                pts[m] = (la, lo)
        if len(pts) != 3:
            return False, None, ages
        ds = []
        ms = list(t.members)
        for i in range(3):
            for j in range(i + 1, 3):
                a, b = pts[ms[i]], pts[ms[j]]
                ds.append(_dist_m(a[0], a[1], b[0], b[1]))
        spread = max(ds) if ds else 999.0
        return spread <= CONSENSUS_PAIR_GATE_M, spread, ages
    ...
    def _task_state_machine(self, obs: SwarmObs, dt: float) -> None:
        ...
        consensus_ready, spread, evidence_ages = self._triad_consensus(t)

        if self._t - self._last_task_status_log_t >= 2.0:
            vis_age = ",".join(f"{m}:{evidence_ages.get(m, 999.0):.1f}" for m in t.members)
            spread_s = "NA" if spread is None else f"{spread:.0f}m"
            print(
                f"[V28][{self.my_uid}] TASK_STATUS t={self._t:.1f}s "
                f"seq={t.seq} phase={t.phase} age={age:.1f}s "
                f"acks={len(t.acks)}/{len(t.members)} ev_age={vis_age} "
                f"cons={'Y' if consensus_ready else 'N'} spread={spread_s} "
                f"ever3={'Y' if t.ever_all_visual else 'N'} proxy={t.proxy_dwell:.1f}s"
            )
            print(
                f"[V28][{self.my_uid}] TRIAD_CONSENSUS t={self._t:.1f}s "
                f"seq={t.seq} ready={'Y' if consensus_ready else 'N'} "
                f"spread={spread_s} gate={CONSENSUS_PAIR_GATE_M:.0f}m"
            )
        ...
        # V26 DWELL requires fresh, pairwise-consistent INDEPENDENT tracks
        # from owner + both followers, rather than three timestamps that were
        # all derived from the shared task predictor.
        if all_acked and consensus_ready:
            ...
            if t.phase == "R" and self._t - t.all_visual_since >= 1.4:
                ...
                print(
                    f"[V28][{self.my_uid}] DWELL_ENTER t={self._t:.1f}s "
    ...
    def _follow_safe_route(
        ...
        *,
        ...
        if (
            ...
            print(
                f"[V28][{self.my_uid}] SAM_PADDED_INSIDE "
                ...
                if kind == "H" and len(parts) >= 5:
                    ...
                    if self._retired_tasks.get(key, -999.0) > self._t:
                        if self._t - self._last_stale_task_log_t >= 3.0:
                            print(
                                f"[V28][{self.my_uid}] STALE_T_DROP "
                    ...
                    if (
                        ...
                        and t.seq == seq
                        and member in t.members
                        and _dist_m(lat, lon, t.filt.lat, t.filt.lon) <= EVIDENCE_REPORT_SANITY_M
                    ):
                        # Store the follower's INDEPENDENT local evidence.
                        # It is not declared same-target here; the triad gate
                        # below performs pairwise consensus across all 3 UAVs.
                        t.visuals[member] = self._t
                        t.evidence_points[member] = (self._t, lat, lon)
                        t.acks[member] = self._t

                        # Navigation predictor may still use only a very weak
                        # fusion when the evidence is already near it.
                        if _dist_m(lat, lon, t.filt.lat, t.filt.lon) <= VISUAL_GATE_M:
                            t.filt.lat = 0.96 * t.filt.lat + 0.04 * lat
                            t.filt.lon = 0.96 * t.filt.lon + 0.04 * lon
